...
Game::Game(int width, int height)
    : width(width), height(height), buffer(width, height), last_frame_time(chrono::steady_clock::now()) {
...
void Game::processInput() {
    ...
    if (!entities.empty()) {
        ...
        if (st.p1_thrust) p1.vy = -10.0f; // arriba
        if (st.p1_fire)   hud.st.p1Score += cfg.scorePerSmall; // suma puntos para ver HUD en vivo
    }
...
void Game::update(float delta_time) {
    for (auto& entity : entities) {
        ...
        wrapInPlay(entity); // Nuevo (los if se reemplazaron con la nueva funcion)
    }
}
